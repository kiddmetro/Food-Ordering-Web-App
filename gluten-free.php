    ...
    <main class="main-body">
        ...
        <section class="dish-section">
            <div class="section-heading">
                <h4 class="section-title">Gluten Free Category </h4>
            </div>
            <div class="dish-wrap">
            <?php
            // Fetch dish data from the "healthy_gluten" table in the database
            $query = "SELECT * FROM glutens";
            $result = mysqli_query($db, $query);

            // Check if any dishes are found
            if (mysqli_num_rows($result) > 0) {
            // Loop through the fetched results and generate HTML code for each dish
            while ($gluten = mysqli_fetch_assoc($result)) {
                $image = $gluten['image'];
                $category = $gluten['gluten_category'];
                $title = $gluten['gluten_title'];
                $portion = $gluten['gluten_portion'];
                $price = $gluten['gluten_price'];
                $time = $gluten['gluten_time'];
                $description = $gluten['gluten_desc'];
            ?>
                <aside class="dish-card">
                    <img src="./images/category/gluten/<?php echo $image; ?>" alt="" class="dish-image">
                     <div class="dish-info">
                        ...
                        <h3 class="dish-title"><?php echo $title; ?></h3>
                            ...
                            <div class="portion-section">
                                ...
                                <select id="portion" name="gluten_portion" class="portion-area">
        ...
        <?php } else {
         ...
         <?php if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_to_cart']) && isset($_POST['dish_id']) && $_POST['dish_id'] == $dish['gluten_id']): ?>
                        <div class="overlay">
                            <div class="modal-box">
                    ...
                    window.location.href = "shopping-cart.php";
                     ...
                     // Clear the form data
                    document.getElementById("addToCartForm_<?php echo $dish['gluten_id']; ?>").reset();
                    </script>
            </div>
        </section>
            </div>
