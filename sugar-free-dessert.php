    ...
    <main class="main-body">
        ...
        <section class="dish-section">
            <div class="section-heading">
                <h4 class="section-title">Sugar Free Dessert Category </h4>
            </div>
            <div class="dish-wrap">
            <?php
            // Fetch dish data from the "vegan" table in the database
            $query = "SELECT * FROM sugardesserts";
            $result = mysqli_query($db, $query);

            // Check if any dishes are found
            if (mysqli_num_rows($result) > 0) {
            // Loop through the fetched results and generate HTML code for each dish
            while ($sugardessert = mysqli_fetch_assoc($result)) {
                $image = $sugardessert['image'];
                $category = $sugardessert['sugardessert_category'];
                $title = $sugardessert['sugardessert_title'];
                $portion = $sugardessert['sugardessert_portion'];
                $price = $sugardessert['sugardessert_price'];
                $time = $sugardessert['sugardessert_time'];
                $description = $sugardessert['sugardessert_desc'];
            ?>
                <aside class="dish-card">
                    <img src="./images/category/sugarfreedessert/<?php echo $image; ?>" alt="" class="dish-image">
                     <div class="dish-info">
                        ...
                        <h3 class="dish-title"><?php echo $title; ?></h3>
                            ...
                            <div class="portion-section">
                                ...
                                <select id="portion" name="sugardessert_portion" class="portion-area">
                        ...
                        <form action="./shopping-cart.php"  id="addToCartForm_<?php echo $sugardessert['sugardessert_id']; ?>" method="POST">
                            <button type="submit" name="add_to_cart" class="add-cart" onclick="addToCart(<?php echo $sugardessert['sugardessert_id']; ?>)"  >Add to Cart</button>
                            <input type="hidden" name="dish_id" value="<?php echo $sugardessert['sugardessert_id']; ?>">
        ...
        <?php } else {
         ...
         <?php if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_to_cart']) && isset($_POST['dish_id']) && $_POST['dish_id'] == $dish['sugardessert_id']): ?>
                        <div class="overlay">
                            <div class="modal-box">
                    ...
                    window.location.href = "shopping-cart.php";
                     ...
                     // Clear the form data
                    document.getElementById("addToCartForm_<?php echo $dish['sugardessert_id']; ?>").reset();
                    </script>
            </div>
        </section>
            </div>
