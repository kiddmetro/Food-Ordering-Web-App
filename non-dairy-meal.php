...
<body>
    ...
    <main class="main-body">
        ...
        <!--HEALTHY nondairy SECTION -->
        <section class="dish-section">
            <div class="section-heading">
                <h4 class="section-title">Non Dairy Category </h4>
            </div>
            <div class="dish-wrap">
            <?php
            ...
            while ($nondairy = mysqli_fetch_assoc($result)) {
                $image = $nondairy['image'];
                $category = $nondairy['nondairy_category'];
                $title = $nondairy['nondairy_title'];
                $portion = $nondairy['nondairy_portion'];
                $price = $nondairy['nondairy_price'];
                $time = $nondairy['nondairy_time'];
                $description = $nondairy['nondairy_desc'];
            ?>
                <aside class="dish-card">
                    <img src="./images/category/nondairy/<?php echo $image; ?>" alt="" class="dish-image">
                     <div class="dish-info">
                        ...
                        <h3 class="dish-title"><?php echo $title; ?></h3>
                            ...
                            <div class="portion-section">
                                ...
                                <select id="portion" name="nondairy_portion" class="portion-area">
                        ...
                        <form action="./shopping-cart.php"  id="addToCartForm_<?php echo $nondairy['nondairy_id']; ?>" method="POST">
                            <button type="submit" name="add_to_cart" class="add-cart" onclick="addToCart(<?php echo $nondairy['nondairy_id']; ?>)"  >Add to Cart</button>
                            <input type="hidden" name="dish_id" value="<?php echo $nondairy['nondairy_id']; ?>">
        ...
        <?php } else {
         ...
         <?php if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_to_cart']) && isset($_POST['dish_id']) && $_POST['dish_id'] == $dish['nondairy_id']): ?>
                        <div class="overlay">
                            <div class="modal-box">
                    ...
                    window.location.href = "shopping-cart.php";
                     ...
                     // Clear the form data
                    document.getElementById("addToCartForm_<?php echo $dish['nondairy_id']; ?>").reset();
                    </script>
            </div>
        </section>
            </div>
