    ...
    <main class="main-body">
        ...
        <section class="dish-section">
            <div class="section-heading">
                <h4 class="section-title">Vegan Category </h4>
            </div>
            <div class="dish-wrap">
            <?php
            // Fetch dish data from the "healthy_vegan" table in the database
            $query = "SELECT * FROM vegans";
            $result = mysqli_query($db, $query);

            // Check if any dishes are found
            if (mysqli_num_rows($result) > 0) {
            // Loop through the fetched results and generate HTML code for each dish
            while ($vegan = mysqli_fetch_assoc($result)) {
                $image = $vegan['image'];
                $category = $vegan['vegan_category'];
                $title = $vegan['vegan_title'];
                $portion = $vegan['vegan_portion'];
                $price = $vegan['vegan_price'];
                $time = $vegan['vegan_time'];
                $description = $vegan['vegan_desc'];
            ?>
                <aside class="dish-card">
                    <img src="./images/category/vegan/<?php echo $image; ?>" alt="" class="dish-image">
                     <div class="dish-info">
                        ...
                        <h3 class="dish-title"><?php echo $title; ?></h3>
                            ...
                            <div class="portion-section">
                                ...
                                <select id="portion" name="vegan_portion" class="portion-area">
                        ...
                        <form action="./shopping-cart.php"  id="addToCartForm_<?php echo $vegan['vegan_id']; ?>" method="POST">
                            <button type="submit" name="add_to_cart" class="add-cart" onclick="addToCart(<?php echo $vegan['vegan_id']; ?>)"  >Add to Cart</button>
                            <input type="hidden" name="dish_id" value="<?php echo $vegan['vegan_id']; ?>">
        ...
        <?php } else {
         ...
         <?php if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_to_cart']) && isset($_POST['dish_id']) && $_POST['dish_id'] == $dish['vegan_id']): ?>
                        <div class="overlay">
                            <div class="modal-box">
                    ...
                    window.location.href = "shopping-cart.php";
                     ...
                     // Clear the form data
                    document.getElementById("addToCartForm_<?php echo $dish['vegan_id']; ?>").reset();
                    </script>
            </div>
        </section>
            </div>
